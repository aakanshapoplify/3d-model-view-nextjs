...
@app.post("/convert/cad-to-glb")
async def convert_cad_to_glb(
    file: UploadFile = File(...),
    px_to_m: float = Form(0.01),
    wall_thickness: float = Form(0.15),
    wall_height: float = Form(3.0),
    ...
    try:
        ...
        file_size = buf.tell()
        buf.seek(0)
        print(f"Generated GLB file from CAD: {file_size} bytes")
        ...
        "formats": [
            {
                "type": "svg",
                "extensions": [".svg"],
                "description": "Scalable Vector Graphics floor plans",
                "endpoint": "/convert/svg-to-glb"
            },
            {
                "type": "jpg",
                "extensions": [".jpg", ".jpeg"],
